...
namespace DRimEditor.DetailView
{
    public static class FieldDescHelper
    {
        ...
        public static List<Pair<FieldInfo, object>> GetAllDirectEditableFieldValues(Type objType, object obj)
        {
        ...
        public static void GenerateFromCommand(Type baseChildType, object obj, ref string str)
        {
            if (str != null && str != "")
                str += ":";

            List<Pair<FieldInfo, object>> pairs = GetAllDirectEditableFieldValues(baseChildType, obj);
            if (pairs == null || pairs.Count == 0)
            {
                Log.Error("Couldn't find any appropriate fields in GenerateFromCommand");
                return;
            }
